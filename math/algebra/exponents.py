...
class Exponent_Laws(Scene):
    def construct(self):
        # self.product_rule()
        # self.product_rule_writing()
        self.quotient_rule()

    def gradients(self):
        japanese_blue_gradient = ["#d4eef5", "#9dd7d8"]
        pass
    ...
    def quotient_rule(self):
        ...
        expanded_exponential_fraction = MathTex(r"a^4/a^3=\frac{a\cdot a \cdot a \cdot a}{a\cdot a \cdot a}")
        exponent_laws_list = [
            product_rule,
            quotient_rule,
            power_rule,
            power_of_quotient_rule,
        ]

        exponent_laws_group = VGroup(*exponent_laws_list).arrange(DOWN)

        def set_colors(laws=exponent_laws_group, color="#0092CC"):
            colored = []
            for law in laws:
                law.set_color(color, 50)
                colored.append(law)
            return colored

        colored_laws = set_colors()

        def write_all_laws(laws=colored_laws):
            for law in laws:
                self.play(Write(law))

        write_all_laws()

        def zoom_on_quotient_rule(quotient_rule=quotient_rule, laws=colored_laws):
            box = SurroundingRectangle(quotient_rule, buff=0.2).set_color(darker_blues, 20)
            quotient_rule_text = (
            ...
            box.add_updater(
                lambda b: b.become(
                    SurroundingRectangle(laws[1], buff=0.2)
                    .set_color(darker_blues, 50)
                    .set_stroke(width=1)
                )
            )
            self.play(Create(box), box.animate.set_stroke(width=2), run_time=0.5)
            self.play(FadeOut(laws[0]), FadeOut(laws[2]), FadeOut(laws[3]))
            self.play(laws[1].animate.center())
            box.clear_updaters()
            self.play(Write(quotient_rule_text), FadeOut(box))

        zoom_on_quotient_rule()
        self.play(ReplacementTransform(colored_laws[1], expanded_exponential_fraction))
